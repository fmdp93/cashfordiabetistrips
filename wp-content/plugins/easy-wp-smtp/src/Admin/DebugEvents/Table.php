...
class Table extends \WP_List_Table {
	...
	public function get_counts() {
		 ...
		 * }
		 */
		$this->counts = apply_filters( 'easy_wp_smtp_admin_debug_events_table_get_counts', $this->counts );
	}
	...
	public function get_views() {
		 ...
		 * }
		 */
		return apply_filters( 'easy_wp_smtp_admin_debug_events_table_get_views', $views, $this->counts );
	}
	...
	public function get_columns() {
		...
		return [
			'event'      => esc_html__( 'Name', 'easy-wp-smtp' ),
			'type'       => esc_html__( 'Type', 'easy-wp-smtp' ),
			'content'    => esc_html__( 'Content', 'easy-wp-smtp' ),
			'initiator'  => esc_html__( 'Source', 'easy-wp-smtp' ),
			'created_at' => esc_html__( 'Date', 'easy-wp-smtp' ),
		];
	}
	...
	public function column_event( $item ) {
		...
		return '<strong>' .
			...
			' class="js-easy-wp-smtp-debug-event-preview row-title event-preview" title="' . esc_attr( $item->get_title() ) . '">' .
	...
	public function no_items() {
		...
		if ( $this->is_filtered() ) {
			esc_html_e( 'No events found.', 'easy-wp-smtp' );
		} else {
			esc_html_e( 'No events have been logged for now.', 'easy-wp-smtp' );
		}
	}
	...
	protected function extra_tablenav( $which ) {
		...
		?>
		<div class="alignleft actions easy-wp-smtp-filter-date">

			<input type="text" name="date" class="regular-text easy-wp-smtp-filter-date-selector easy-wp-smtp-filter-date__control"
						 placeholder="<?php esc_attr_e( 'Select a date range', 'easy-wp-smtp' ); ?>"
						 value="<?php echo esc_attr( $date ); ?>">

			<button type="submit" name="action" value="filter_date" class="button easy-wp-smtp-filter-date__btn">
				<?php esc_html_e( 'Filter', 'easy-wp-smtp' ); ?>
			</button>

		</div>
		<?php
		if ( current_user_can( 'manage_options' ) ) {
			wp_nonce_field( 'easy_wp_smtp_debug_events', 'easy-wp-smtp-debug-events-nonce', false );
			printf(
				'<button id="easy-wp-smtp-delete-all-debug-events-button" type="button" class="button">%s</button>',
				esc_html__( 'Delete All Events', 'easy-wp-smtp' )
			);
		}
	}
